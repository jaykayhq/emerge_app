...
void main() {
  ...
  group('App Startup Tests', () {
    testWidgets('App starts and shows Login screen', (
      WidgetTester tester,
    ) async {
      // Start the app
  ...
  group('Notification Flow Integration Tests', () {
    ...
    setUp(() {
      ...
      when(
        () => mockNotificationService.notifyHabitCreated(any(), any()),
      ).thenAnswer((_) async {});
    });
    ...
    testWidgets('Scholar archetype: Creates habit with notification scheduling', (
      ...
      expect(
        timelineIconFinder,
        findsOneWidget,
        reason: 'Timeline icon should be visible',
      );
      ...
      expect(
        fabFinder,
        findsOneWidget,
        reason: 'FAB should be visible on Timeline',
      );
      ...
      expect(
        setTimeButtonFinder,
        findsOneWidget,
        reason: 'Set Time button should be visible',
      );
      ...
      expect(
        createButtonFinder,
        findsOneWidget,
        reason: 'Create Habit button should be visible',
      );
      ...
      verify(
        () => mockNotificationService.scheduleHabitReminder(
          ...
          any<String>(
            that: contains('08'),
          ), // reminderTime should contain '08' for 8 AM
          any(), // frequency
          any(), // specificDays
        ),
      ).called(1);
    ...
    testWidgets(
      'User without archetype: Creates habit with fallback time (7 AM)',
      (WidgetTester tester) async {
        ...
        verify(
          () => mockNotificationService.scheduleHabitReminder(
            ...
            any<String>(
              that: contains('07'),
            ), // Should be 07:XX for 7 AM fallback
            any(),
            any(),
          ),
        ).called(1);
    ...
    testWidgets(
      'Multiple habits: Creates two habits with correct notification times',
      (WidgetTester tester) async {
        // Arrange: Create a Scholar user
        final scholarProfile = UserProfile(
          uid: 'test-multi-habit-user',
          avatarStats: const UserAvatarStats(level: 5, intellectXp: 500),
          archetype: UserArchetype.scholar,
          onboardingProgress: 4,
        );
