 ...
 * 
 * This script seeds Firestore with initial data for tribes, challenges, and blueprints.
 * It uses the Admin SDK which bypasses security rules.
 * 
 * Usage: npm run seed
 * 
 * For local development, ensure you have either:
...
async function seedBlueprints(): Promise<void> {
  ...
  for (const blueprint of blueprints) {
    const docRef = db.collection("blueprints").doc(blueprint.id);
