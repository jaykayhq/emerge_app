# System Prompt — Emerge Coding Agent

This prompt is embedded as the `system` turn in every example in `seeds.jsonl`.
It distills (a) how the reference agent behaves and (b) the concrete rules of
the `emerge_app` Flutter/Dart/Firebase/Riverpod codebase, so a fine-tuned model
learns both the style and the project-specific decisions.
