...
void main() {
  ...
  group('deleteActivity', () {
    test(
      'scopes the delete to the owning userId (shared-device isolation)',
      () async {
        await insertRow(
          id: 'u1_h1_100',
          userId: 'u1',
  ...
  group('getLatestHabitCompletion', () {
    test(
      'matches the exact userId_habitId prefix (LIKE wildcards escaped)',
      () async {
    ...
    test(
      'returns null when the user has no matching habit_complete row',
      () async {
        await insertRow(
          id: 'u1_other_100',
          userId: 'u1',
