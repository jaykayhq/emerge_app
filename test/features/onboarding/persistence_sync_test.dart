...
void main() {
  ...
  setUp(() {
    ...
    // Register fallback for UserProfile
    ...
    container = ProviderContainer(
      overrides: [
        ...
        // Mock auth state with identity fields
        ...
        userProfileProvider.overrideWithValue(
          AsyncValue.data(const UserProfile(uid: 'test-user')),
        ),
      ],
    );
    ...
    when(() => mockUserStatsRepo.saveUserStats(any())).thenAnswer((_) async {});
    when(
  ...
  group('Identity Persistence Sync Tests', () {
    test(
      'selectArchetype persists identity fields via UserProfileRepository',
      () async {
        ...
        final capturedProfile =
            verify(
                  () => mockUserProfileRepo.updateProfile(captureAny()),
                ).captured.first
                as UserProfile;

        expect(capturedProfile.uid, 'test-user');
        expect(capturedProfile.displayName, 'Test User');
        expect(capturedProfile.photoUrl, 'https://example.com/photo.jpg');
        expect(capturedProfile.archetype, UserArchetype.athlete);
      },
    );

    test(
      'completeMilestone persists identity fields via UserProfileRepository',
      () async {
        final notifier = container.read(enhancedOnboardingProvider.notifier);
