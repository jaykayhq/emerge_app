...
void main() {
  group('BlueprintRepository', () {
    test('incrementAdoptionCount increments field on an existing doc', () async {
      ...
      await firestore.collection('blueprints').doc('test_blueprint_id').set({
        'adoptionCount': 5,
      });
    ...
    test(
      'seedBlueprintsIfEmpty seeds default blueprints when collection is empty',
      () async {
        final firestore = FakeFirebaseFirestore();
        final repo = BlueprintRepository(firestore);
    ...
    test(
      'seedBlueprintsIfEmpty skips seeding when v6 data already exists',
      () async {
        final firestore = FakeFirebaseFirestore();
    ...
    test(
      ...
      () async {
        ...
        final morning1 = snap.docs
            .firstWhere((d) => d.id == 'morning_1')
            .data();
      ...
      () async {
        ...
        for (final doc in snap.docs) {
          ...
          expect(
            ...
            isNotEmpty,
            reason:
                '${doc.id} should have a non-empty recommendedArchetypes '
                'list',
          );
        }
      },
    ...
    test(
      ...
      () async {
        ...
        expect(snap.docs.length, _seedBlueprintIds.length);
        for (final doc in snap.docs) {
          final habits = (doc.data()['habits'] as List)
              .cast<Map<String, dynamic>>();
          expect(habits, isNotEmpty, reason: '${doc.id} should carry habits');
          for (final habit in habits) {
            expect(
              habit['timeOfDay'],
              isNotNull,
              reason:
                  '${doc.id} habit "${habit['title']}" should carry a '
                  'timeOfDay',
            );
            expect(
              habit['attribute'],
              isNotNull,
              reason:
                  '${doc.id} habit "${habit['title']}" should carry an '
                  'attribute',
            );
          }
        }
    ...
    test(
      'seedBlueprintsIfEmpty writes slot metadata on morning_1 habits',
      () async {
        final firestore = FakeFirebaseFirestore();
        final repo = BlueprintRepository(firestore);
        ...
        final snap = await firestore
            ...
            .doc('morning_1')
            .get();
        final habits = (snap.data()?['habits'] as List)
            .cast<Map<String, dynamic>>();
    ...
    test(
      ...
      () async {
        ...
        final habits = (morning1['habits'] as List)
            .cast<Map<String, dynamic>>();
