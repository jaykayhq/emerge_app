...
void main() {
  group('IdentityEngine', () {
    ...
    test(
      'calculateDominantArchetype returns correct archetype for single vote',
      () {
        final result = IdentityEngine.calculateDominantArchetype({
          'scholar': 5,
        });
    ...
    test(
      'calculateDominantArchetype returns correct archetype for multiple votes',
      () {
        final result = IdentityEngine.calculateDominantArchetype({
          'athlete': 2,
          'creator': 10,
    ...
    test(
      'calculateDominantArchetype resolves ties deterministically (alphabetical)',
      () {
        final result = IdentityEngine.calculateDominantArchetype({
          'scholar': 5,
          'athlete': 5,
    ...
    test('calculateDominantArchetype returns none for invalid keys', () {
      final result = IdentityEngine.calculateDominantArchetype({'wizard': 10});
      expect(result, UserArchetype.none);
    });
  });
}
