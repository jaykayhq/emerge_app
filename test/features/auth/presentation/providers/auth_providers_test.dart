...
Future<AuthUser> _readFirstAuthState(ProviderContainer container) {
  ...
  container.listen<AsyncValue<AuthUser>>(authStateChangesProvider, (
    previous,
    next,
  ) {
...
Future<bool> _readFirstBoolState(
  ProviderContainer container,
  dynamic provider,
) {
  final completer = Completer<bool>();
  final sub = container.listen<AsyncValue<bool>>(provider, (previous, next) {
...
void main() {
  ...
  group('authStateChangesProvider', () {
    test('emits user stream from repository', () async {
      when(() => mockRepo.user).thenAnswer(
        (_) =>
            Stream.value(const AuthUser(id: 'test', email: 'test@example.com')),
      );
    ...
    test('emits anonymous when repo emits empty', () async {
      when(() => mockRepo.user).thenAnswer((_) => Stream.value(AuthUser.empty));
  ...
  group('signInProvider', () {
    test('calls repository signInWithEmailAndPassword', () async {
      when(
        () => mockRepo.signInWithEmailAndPassword(
          email: any(named: 'email'),
          password: any(named: 'password'),
        ),
      ).thenAnswer(
        (_) async => Right(const AuthUser(id: 'test', email: 'test@test.com')),
      );
  ...
  group('role check providers', () {
    ...
    test(
      'isNormalUserProvider returns true if users/{uid} document exists',
      () async {
        ...
        final result = await container.read(
          isNormalUserProvider('user123').future,
        );
    ...
    test(
      'isNormalUserProvider returns false if users/{uid} document does not exist',
      () async {
        final container = ProviderContainer(
          overrides: [firestoreProvider.overrideWithValue(fakeFirestore)],
        );
    ...
    test(
      'isCreatorProvider returns true if creator_profiles/{uid} document exists',
      () async {
        await fakeFirestore
        ...
        final result = await container.read(
          isCreatorProvider('creator123').future,
        );
        expect(result, isTrue);
        container.dispose();
      },
    ...
    test(
      'signUpCreatorProvider override with error propagates error',
      () async {
        final container = ProviderContainer(
          overrides: [
    ...
    test(
      'isCreatorProvider returns false if creator_profiles/{uid} document does not exist',
      () async {
        final container = ProviderContainer(
          overrides: [firestoreProvider.overrideWithValue(fakeFirestore)],
        );
    ...
    test(
      'isNormalUserProvider returns false immediately if uid is empty or whitespace',
      () async {
        final container = ProviderContainer(
          overrides: [firestoreProvider.overrideWithValue(fakeFirestore)],
        );
        ...
        expect(
          await container.read(isNormalUserProvider('   ').future),
          isFalse,
        );
        container.dispose();
      },
    ...
    test(
      'isCreatorProvider returns false immediately if uid is empty or whitespace',
      () async {
        final container = ProviderContainer(
          overrides: [firestoreProvider.overrideWithValue(fakeFirestore)],
        );
    ...
    test(
      'isCurrentNormalUserProvider returns true if logged in and users/{uid} doc exists',
      () async {
        await fakeFirestore.collection('users').doc('current123').set({
          'name': 'Current Normal',
        });
        ...
        final container = ProviderContainer(
          overrides: [
            authStateChangesProvider.overrideWith(
              (ref) => Stream.value(
                const AuthUser(id: 'current123', email: 'current@test.com'),
              ),
            ),
        ...
        final result = await _readFirstBoolState(
          container,
          isCurrentNormalUserProvider,
        );
        expect(result, isTrue);
        container.dispose();
      },
    ...
    test(
      'isCurrentNormalUserProvider returns false if not logged in or users/{uid} doc does not exist',
      () async {
        final container = ProviderContainer(
          overrides: [
            authStateChangesProvider.overrideWith(
              (ref) => Stream.value(AuthUser.empty),
            ),
            firestoreProvider.overrideWithValue(fakeFirestore),
          ],
        );
    ...
    test(
      'isCurrentCreatorProvider returns true if logged in and creator_profiles/{uid} doc exists',
      () async {
        await fakeFirestore
            .collection('creator_profiles')
            .doc('current123')
            .set({'name': 'Current Creator'});
    ...
    test(
      'isCurrentCreatorProvider returns false if not logged in or creator_profiles/{uid} doc does not exist',
      () async {
        final container = ProviderContainer(
          overrides: [
            authStateChangesProvider.overrideWith(
