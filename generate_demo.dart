...
void main() async {
  ...
  AppLogger.d('Starting authenticated image generation for overhead maps...');
  ...
  for (final entry in prompts.entries) {
    ...
    try {
      AppLogger.d('Downloading $filename. This might take 30-120 seconds...');
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.d('Saved $filename successfully.');
      } else {
        AppLogger.e('Failed $filename: HTTP ${response.statusCode}');
        final bodyBytes = await response.toList();
        final body = String.fromCharCodes(bodyBytes.expand((x) => x));
        AppLogger.e('Response body: $body');
      }
    } catch (e) {
      AppLogger.e('Failed $filename with error: $e');
    }
  }

  client.close();
  AppLogger.d('Finished generation process.');
}
