...
1. **Web Google sign-in root cause = API key referrer restriction.** The dedicated web key (`AIza… (ID b3783a74-…)`, value redacted here; restricted this morning in `b107bd26`) allowed ONLY `tradeflash-l2966.web.app` + `emerge-404.web.app`. Verified live: identitytoolkit returns `403 "Requests from referer ... are blocked"` for `localhost` and `emerge.app` referers, while the web.app referer passed. **Fixed via gcloud (owner account `joeukpai55@gmail.com` active):** added `http://localhost/*`, `http://localhost:*/*`, `https://localhost:*/*`, `https://tradeflash-l2966-*.web.app/*` (preview channels). Re-verified live: localhost passes (400 endpoint-reached), parked `emerge.app` still blocked. The web key's referrer restrictions are applied and held in the GCP console — `scripts/restrict_api_keys.sh` no longer manages web referrers (its now-removed `WEB_REFERRERS` variable was dropped when the dedicated web key moved to console-side restriction). If the user tests web on a custom domain, it must be added in the console too.
...
- API key restriction (console-only, owner action): Android key, iOS key (values redacted here), web key now dedicated per commit `b107bd26` — apply Application + API restrictions to all three, then resolve the 3 secret-scanning alerts.
