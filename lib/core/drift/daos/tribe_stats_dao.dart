...
class TribeStatsDao extends DatabaseAccessor<AppDatabase>
    with _$TribeStatsDaoMixin {
  ...
  Future<TribeStatsTableData?> getByArchetype(String archetypeId) {
    return (select(
      tribeStatsTable,
    )..where((t) => t.archetypeId.equals(archetypeId))).getSingleOrNull();
  }

  Future<TribeStatsTableData?> getStats(String tribeId) {
    return (select(
      tribeStatsTable,
    )..where((t) => t.tribeId.equals(tribeId))).getSingleOrNull();
  }

  Stream<TribeStatsTableData?> watchStats(String tribeId) {
    return (select(
      tribeStatsTable,
    )..where((t) => t.tribeId.equals(tribeId))).watchSingleOrNull();
  }
    ...
    await upsertStats(
      TribeStatsTableCompanion(
        ...
        totalChallengesCompleted: Value(
          current.totalChallengesCompleted + challenges,
        ),
        ...
        userChallengesCompleted: Value(
          current.userChallengesCompleted + challenges,
        ),
