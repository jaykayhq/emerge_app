...
class HabitCompletionsDao extends DatabaseAccessor<AppDatabase> with _$HabitCompletionsDaoMixin {
  ...
  Future<void> insertFromData({
    ...
    return into(habitCompletionsTable).insert(HabitCompletionsTableCompanion(
      id: Value(id),
  ...
  Future<List<HabitCompletionsTableData>> getBetweenDates(String userId, String start, String end) {
    return (select(habitCompletionsTable)
      ..where((t) => t.userId.equals(userId) & t.completedAt.isBetweenValues(start, end)))
      .get();
  }
