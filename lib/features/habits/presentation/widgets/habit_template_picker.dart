import 'package:emerge_app/core/theme/app_theme.dart';
import 'package:emerge_app/core/theme/archetype_theme.dart';
import 'package:emerge_app/features/auth/domain/entities/user_extension.dart';
import 'package:emerge_app/features/habits/domain/entities/habit.dart';
import 'package:emerge_app/features/timeline/presentation/widgets/habit_timeline_section.dart';
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';
import 'package:gap/gap.dart';

/// Callback when a habit template is selected
typedef OnTemplateSelected = void Function(HabitTemplate template);

/// Extended habit template with all creation fields
class HabitTemplate {
  final String title;
  final String description;
  final String anchor;
  final String category;
  final String emoji; // Changed from icon to emoji
  final String frequency;
  final String timeOfDay;
  final HabitAttribute attribute;
  final TimeOfDay? defaultTime;
  final TimeOfDayPreference? timeOfDayPreference;
  final int timerDurationMinutes; // 0 = no timer
  final HabitIntegrationType integrationType;
  final int? integrationTarget;

  const HabitTemplate({
    required this.title,
    required this.description,
    required this.anchor,
    required this.category,
    required this.emoji,
    this.frequency = 'Daily',
    this.timeOfDay = 'Morning',
    this.attribute = HabitAttribute.vitality,
    this.defaultTime,
    this.timeOfDayPreference,
    this.timerDurationMinutes = 0,
    this.integrationType = HabitIntegrationType.none,
    this.integrationTarget,
  });

  /// Get the identity color for this template's attribute
  Color get color => attributeColor(attribute);

  /// Whether this template has a timer
  bool get hasTimer => timerDurationMinutes > 0;

  /// Whether this template has health integration
  bool get hasHealthIntegration => integrationType != HabitIntegrationType.none;

  /// Convert from ArchetypeHabitSuggestion
  factory HabitTemplate.fromSuggestion(
    ArchetypeHabitSuggestion suggestion, {
    String category = 'General',
  }) {
    return HabitTemplate(
      title: suggestion.title,
      description: suggestion.description,
      anchor: suggestion.anchor,
      category: category,
      emoji: _getEmojiForIcon(suggestion.icon), // Convert icon to emoji
      defaultTime: suggestion.defaultTime,
      timeOfDayPreference: suggestion.timeOfDayPreference,
    );
  }

  /// Helper to map IconData to emoji string
  static String _getEmojiForIcon(IconData icon) {
    // Map common icons to their emoji equivalents
    final iconMap = {
      Icons.fitness_center: '💪',
      Icons.water_drop: '💧',
      Icons.directions_walk: '🚶',
      Icons.sports_gymnastics: '🏋️',
      Icons.bedtime: '😴',
      Icons.spa: '🧘',
      Icons.favorite: '❤️',
      Icons.air: '🌬️',
      Icons.phone_disabled: '📵',
      Icons.nights_stay: '🌙',
      Icons.menu_book: '📖',
      Icons.school: '🎓',
      Icons.podcasts: '🎙️',
      Icons.edit_note: '✍️',
      Icons.psychology: '🧠',
      Icons.checklist: '✅',
      Icons.timer: '⏱️',
      Icons.email: '📧',
      Icons.calendar_month: '📅',
      Icons.today: '📆',
      Icons.palette: '🎨',
      Icons.lightbulb: '💡',
      Icons.rocket_launch: '🚀',
      Icons.explore: '🔍',
      Icons.brush: '🖌️',
    };

    return iconMap[icon] ?? '🔥';
  }
}

/// Inline horizontal carousel showing archetype-specific habit templates
/// Redesigned with cosmic glassmorphism aesthetic
class HabitTemplateCarousel extends StatelessWidget {
  final UserArchetype archetype;
  final OnTemplateSelected onTemplateSelected;
  final VoidCallback? onSeeMore;

  /// Optional list of user-selected interest ids (from `Interest.catalog`).
  /// When supplied, suggestions whose title/description overlap the user's
  /// interest-category vocabulary float to the top of the carousel.
  /// Pass `[]` or null to keep the original archetype order.
  final List<String>? userInterests;

  const HabitTemplateCarousel({
    super.key,
    required this.archetype,
    required this.onTemplateSelected,
    this.onSeeMore,
    this.userInterests,
  });

  String _getArchetypeEmoji(UserArchetype archetype) {
    switch (archetype) {
      case UserArchetype.athlete:
        return '🏃‍➡️';
      case UserArchetype.creator:
        return '🖌️';
      case UserArchetype.scholar:
        return '📖';
      case UserArchetype.stoic:
        return '🧘';
      case UserArchetype.zealot:
        return '🔥';
      default:
        return '🔥';
    }
  }

  @override
  Widget build(BuildContext context) {
    // If archetype hasn't loaded yet (none), fall back to athlete templates
    final effectiveArchetype = archetype == UserArchetype.none
        ? UserArchetype.athlete
        : archetype;
    final theme = ArchetypeTheme.forArchetype(effectiveArchetype);
    final interestCategories = _resolveInterestCategories(userInterests);
    final templates = _rankedTemplates(theme, interestCategories);

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Padding(
          padding: const EdgeInsets.symmetric(horizontal: 16),
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Expanded(
                child: Row(
                  children: [
                    Container(
                      padding: const EdgeInsets.all(6),
                      decoration: BoxDecoration(
                        color: theme.primaryColor.withValues(alpha: 0.15),
                        borderRadius: BorderRadius.circular(8),
                      ),
                      child: Text(
                        _getArchetypeEmoji(archetype),
                        style: const TextStyle(fontSize: 16),
                      ),
                    ),
                    const Gap(10),
                    Flexible(
                      child: Text(
                        'Quick Start',
                        style: Theme.of(context).textTheme.titleSmall?.copyWith(
                          color: AppTheme.textMainDark,
                          fontWeight: FontWeight.w600,
                        ),
                        maxLines: 1,
                        overflow: TextOverflow.ellipsis,
                      ),
                    ),
                  ],
                ),
              ),
              GestureDetector(
                onTap: () {
                  HapticFeedback.lightImpact();
                  if (onSeeMore != null) {
                    onSeeMore!();
                  } else {
                    _showAllTemplates(context, theme);
                  }
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(
                    horizontal: 12,
                    vertical: 6,
                  ),
                  decoration: BoxDecoration(
                    color: theme.primaryColor.withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(16),
                    border: Border.all(
                      color: theme.primaryColor.withValues(alpha: 0.3),
                    ),
                  ),
                  child: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Text(
                        'See All',
                        style: TextStyle(
                          color: theme.primaryColor,
                          fontWeight: FontWeight.w600,
                          fontSize: 12,
                        ),
                      ),
                      const Gap(4),
                      Icon(
                        Icons.arrow_forward_ios,
                        color: theme.primaryColor,
                        size: 10,
                      ),
                    ],
                  ),
                ),
              ),
            ],
          ),
        ),
        const Gap(12),
        SizedBox(
          height: 95, // Reduced from 115 for earthy theme compact design
          child: ListView.separated(
            scrollDirection: Axis.horizontal,
            padding: const EdgeInsets.symmetric(horizontal: 16),
            itemCount: templates.length,
            separatorBuilder: (_, _) => const Gap(12),
            itemBuilder: (context, index) {
              final template = templates[index];
              return _TemplateCard(
                template: template,
                accentColor: template.color,
                onTap: () {
                  HapticFeedback.mediumImpact();
                  onTemplateSelected(template);
                },
              );
            },
          ),
        ),
      ],
    );
  }

  void _showAllTemplates(BuildContext context, ArchetypeTheme theme) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => HabitTemplateSheet(
        archetype: archetype,
        onTemplateSelected: (template) {
          Navigator.of(context).pop();
          onTemplateSelected(template);
        },
      ),
    );
  }

  /// Translate interest ids into the matching `InterestCategory` set so we
  /// can score templates against the user's personalization signal.
  static Set<String> _resolveInterestCategories(List<String>? interestIds) {
    if (interestIds == null || interestIds.isEmpty) return const {};
    // The category prefix is the literal slug before the first dot, e.g.
    // 'movement' for 'movement.walking'.
    return interestIds
        .map((id) => id.split('.').first)
        .where((slug) => slug.isNotEmpty)
        .toSet();
  }

  /// Reorder templates so those whose descriptive text contains a
  /// matching interest-category keyword float to the top.
  static List<HabitTemplate> _rankedTemplates(
    ArchetypeTheme theme,
    Set<String> interestCategories,
  ) {
    final all = theme.suggestedHabits
        .map(
          (h) => HabitTemplate.fromSuggestion(h, category: theme.archetypeName),
        )
        .toList(growable: true);

    if (interestCategories.isEmpty) return all;

    int score(HabitTemplate t) {
      final haystack = '${t.title} ${t.description} ${t.anchor}'.toLowerCase();
      var s = 0;
      for (final cat in interestCategories) {
        // Category slug words appear in template copy (e.g. "movement",
        // "reading", "meditation", "writing") — boosted only on explicit
        // overlap so we don't reorder for unrelated interests.
        if (haystack.contains(cat.toLowerCase())) s += 10;
      }
      return s;
    }

    all.sort((a, b) {
      final cmp = score(b).compareTo(score(a));
      if (cmp != 0) return cmp;
      return a.title.compareTo(b.title);
    });
    return all;
  }
}

/// Individual template card with glassmorphism design
class _TemplateCard extends StatelessWidget {
  final HabitTemplate template;
  final Color accentColor;
  final VoidCallback onTap;

  const _TemplateCard({
    required this.template,
    required this.accentColor,
    required this.onTap,
  });

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        width: 140,
        padding: const EdgeInsets.all(8),
        decoration: BoxDecoration(
          color: Colors.white.withValues(alpha: 0.05),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(
            color: Colors.white.withValues(alpha: 0.1),
            width: 1,
          ),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          mainAxisAlignment: MainAxisAlignment.center,
          mainAxisSize: MainAxisSize.min,
          children: [
            Container(
              padding: const EdgeInsets.all(6),
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [
                    accentColor.withValues(alpha: 0.2),
                    accentColor.withValues(alpha: 0.1),
                  ],
                ),
                borderRadius: BorderRadius.circular(8),
                boxShadow: [
                  BoxShadow(
                    color: accentColor.withValues(alpha: 0.2),
                    blurRadius: 8,
                    spreadRadius: -2,
                  ),
                ],
              ),
              child: Text(template.emoji, style: const TextStyle(fontSize: 16)),
            ),
            const Gap(8),
            Text(
              template.title,
              style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                color: AppTheme.textMainDark,
                fontWeight: FontWeight.w600,
                fontSize: 12,
              ),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
            ),
            const Gap(2),
            Text(
              template.anchor,
              style: Theme.of(context).textTheme.bodySmall?.copyWith(
                color: AppTheme.textSecondaryDark,
                fontSize: 11,
              ),
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
            ),
          ],
        ),
      ),
    );
  }
}

/// Bottom sheet with categorized habit templates - cosmic glassmorphism design
/// Shared template library used by the template sheet and the
/// create-screen typeahead recommendations.
const List<HabitTemplate> habitTemplateCatalog = <HabitTemplate>[
  // ═══ STRENGTH (Physical Power) ═══
  HabitTemplate(
    title: 'Kettlebell Swings',
    description: 'Perform 50 swings for explosive power',
    anchor: 'After arriving home',
    category: 'Strength',
    emoji: '💪',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Pull-up Practice',
    description: '3 sets to failure',
    anchor: 'Before lunch',
    category: 'Strength',
    emoji: '🏋️',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Push-up Blitz',
    description: '20 push-ups',
    anchor: 'After morning coffee',
    category: 'Strength',
    emoji: '🔥',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Deep Squats',
    description: '30 controlled reps',
    anchor: 'After work',
    category: 'Strength',
    emoji: '🦵',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Plank Hold',
    description: 'Hold for 2 minutes',
    anchor: 'Before shower',
    category: 'Strength',
    emoji: '🧱',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 2,
  ),
  HabitTemplate(
    title: 'Deadlift Form',
    description: '10 light technical reps',
    anchor: 'At the gym',
    category: 'Strength',
    emoji: '⚖️',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Core Crusher',
    description: '5 minute abs circuit',
    anchor: 'Before bed',
    category: 'Strength',
    emoji: '🛡️',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Grip Strength',
    description: 'Use crushers for 5 mins',
    anchor: 'While reading',
    category: 'Strength',
    emoji: '✊',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Resistance Work',
    description: '15 mins band exercises',
    anchor: 'Morning ritual',
    category: 'Strength',
    emoji: '🎗️',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'HIIT Sprints',
    description: '5 sets of 30s sprints',
    anchor: 'At the park',
    category: 'Strength',
    emoji: '⚡',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Burpee Burn',
    description: '15 burpees',
    anchor: 'After waking up',
    category: 'Strength',
    emoji: '💥',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Heavy Carry',
    description: '2 min farmers walk',
    anchor: 'During workout',
    category: 'Strength',
    emoji: '📦',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 2,
  ),
  HabitTemplate(
    title: 'Leg Day Ritual',
    description: 'Focus on quad power',
    anchor: 'Tuesday morning',
    category: 'Strength',
    emoji: '🏋️‍♂️',
    attribute: HabitAttribute.strength,
  ),
  HabitTemplate(
    title: 'Upper Body Pump',
    description: '10 mins of dumbbell curls',
    anchor: 'After work',
    category: 'Strength',
    emoji: '💪',
    attribute: HabitAttribute.strength,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Explosive Jumps',
    description: '10 box jumps',
    anchor: 'Before cardio',
    category: 'Strength',
    emoji: '🦘',
    attribute: HabitAttribute.strength,
  ),

  // ═══ VITALITY (Energy & Health) ═══
  HabitTemplate(
    title: 'Cold Shower',
    description: '2 minutes of cold exposure',
    anchor: 'During morning shower',
    category: 'Vitality',
    emoji: '❄️',
    attribute: HabitAttribute.vitality,
    timerDurationMinutes: 2,
  ),
  HabitTemplate(
    title: 'Sunlight Intake',
    description: '10 mins direct sunlight',
    anchor: 'Right after waking',
    category: 'Vitality',
    emoji: '☀️',
    attribute: HabitAttribute.vitality,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Protein Fuel',
    description: 'Eat 30g of protein',
    anchor: 'Breakfast time',
    category: 'Vitality',
    emoji: '🥚',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'No Added Sugar',
    description: 'Avoid refined sugars',
    anchor: 'Daily goal',
    category: 'Vitality',
    emoji: '🚫',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Evening Calm Tea',
    description: 'Drink herbal tea',
    anchor: 'After dinner',
    category: 'Vitality',
    emoji: '🍵',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Mobility Flow',
    description: '10 mins of joints mobility',
    anchor: 'Before work',
    category: 'Vitality',
    emoji: '🧘',
    attribute: HabitAttribute.vitality,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Zone 2 Cardio',
    description: '20 mins steady pace',
    anchor: 'Afternoon break',
    category: 'Vitality',
    emoji: '🏃',
    attribute: HabitAttribute.vitality,
    timerDurationMinutes: 20,
  ),
  HabitTemplate(
    title: 'Standing Desk',
    description: 'Work standing for 1 hour',
    anchor: 'Start of workday',
    category: 'Vitality',
    emoji: '🧍',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Sleep Hygiene',
    description: 'Dim lights 1hr before bed',
    anchor: 'Evening routine',
    category: 'Vitality',
    emoji: '😴',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Magnesium Intake',
    description: 'Take nightly supplement',
    anchor: 'Before bed',
    category: 'Vitality',
    emoji: '💊',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Nature Walk',
    description: '15 mins in green space',
    anchor: 'After lunch',
    category: 'Vitality',
    emoji: '🌳',
    attribute: HabitAttribute.vitality,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'Hydration Anchor',
    description: 'Drink 500ml water',
    anchor: 'After waking up',
    category: 'Vitality',
    emoji: '💧',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Blue Light Filter',
    description: 'Enable night shift mode',
    anchor: '8:00 PM',
    category: 'Vitality',
    emoji: '📱',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Deep Sleep Ritual',
    description: 'No caffeine after 2pm',
    anchor: 'Afternoon',
    category: 'Vitality',
    emoji: '🌙',
    attribute: HabitAttribute.vitality,
  ),
  HabitTemplate(
    title: 'Vitamin D Check',
    description: 'Take daily supplement',
    anchor: 'With breakfast',
    category: 'Vitality',
    emoji: '🌞',
    attribute: HabitAttribute.vitality,
  ),

  // ═══ SPIRIT (Mindfulness) ═══
  HabitTemplate(
    title: 'Metta Meditation',
    description: '5 mins of loving-kindness',
    anchor: 'Before work',
    category: 'Spirit',
    emoji: '💖',
    attribute: HabitAttribute.spirit,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Silence Practice',
    description: '10 mins of total silence',
    anchor: 'After waking up',
    category: 'Spirit',
    emoji: '🤫',
    attribute: HabitAttribute.spirit,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Random Kindness',
    description: 'Help one stranger',
    anchor: 'During the day',
    category: 'Spirit',
    emoji: '🤝',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Gratitude Journal',
    description: 'Write 3 specific things',
    anchor: 'Before bed',
    category: 'Spirit',
    emoji: '📓',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Forgiveness Work',
    description: 'Release one resentment',
    anchor: 'Evening reflection',
    category: 'Spirit',
    emoji: '🕊️',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Deep Breathing',
    description: '5 box breaths',
    anchor: 'Before stress',
    category: 'Spirit',
    emoji: '🌬️',
    attribute: HabitAttribute.spirit,
    timerDurationMinutes: 3,
  ),
  HabitTemplate(
    title: 'Purpose Review',
    description: 'Read your mission statement',
    anchor: 'Morning coffee',
    category: 'Spirit',
    emoji: '🎯',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Serving Others',
    description: 'Give 15 mins to a cause',
    anchor: 'Weekly goal',
    category: 'Spirit',
    emoji: '🤲',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Mindful Eating',
    description: 'No screens during dinner',
    anchor: 'Dinner time',
    category: 'Spirit',
    emoji: '🍽️',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Digital Detox',
    description: 'No phone for 30 mins',
    anchor: 'After work',
    category: 'Spirit',
    emoji: '📵',
    attribute: HabitAttribute.spirit,
    timerDurationMinutes: 30,
  ),
  HabitTemplate(
    title: 'Community Check',
    description: 'Call a loved one',
    anchor: 'Evening walk',
    category: 'Spirit',
    emoji: '📞',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Daily Prayer',
    description: '5 mins of connection',
    anchor: 'Sunrise',
    category: 'Spirit',
    emoji: '🙏',
    attribute: HabitAttribute.spirit,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Nature Connect',
    description: 'Touch a plant/tree',
    anchor: 'Outdoor break',
    category: 'Spirit',
    emoji: '🌿',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Self-Reflection',
    description: 'Review your virtues',
    anchor: 'End of day',
    category: 'Spirit',
    emoji: '🪞',
    attribute: HabitAttribute.spirit,
  ),
  HabitTemplate(
    title: 'Letting Go',
    description: 'Identify one worry to drop',
    anchor: 'After work',
    category: 'Spirit',
    emoji: '🎈',
    attribute: HabitAttribute.spirit,
  ),

  // ═══ INTELLECT (Learning) ═══
  HabitTemplate(
    title: 'Speed Reading',
    description: 'Practice for 5 minutes',
    anchor: 'During commute',
    category: 'Intellect',
    emoji: '⚡',
    attribute: HabitAttribute.intellect,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Summary Writing',
    description: 'Summarize one article',
    anchor: 'After reading',
    category: 'Intellect',
    emoji: '📝',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Language Cards',
    description: '10 Anki flashcards',
    anchor: 'Wait times',
    category: 'Intellect',
    emoji: '🔤',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Logic Puzzle',
    description: 'One Sudoku or riddle',
    anchor: 'Lunch break',
    category: 'Intellect',
    emoji: '🧩',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Chess Blitz',
    description: 'Play one 5min game',
    anchor: 'After dinner',
    category: 'Intellect',
    emoji: '♟️',
    attribute: HabitAttribute.intellect,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Non-fiction Chapter',
    description: 'Read 10 pages',
    anchor: 'Morning coffee',
    category: 'Intellect',
    emoji: '📖',
    attribute: HabitAttribute.intellect,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'Documentary Clip',
    description: 'Watch 15 mins of info',
    anchor: 'Evening rest',
    category: 'Intellect',
    emoji: '📺',
    attribute: HabitAttribute.intellect,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'Note Taking',
    description: 'Process 3 raw notes',
    anchor: 'After work',
    category: 'Intellect',
    emoji: '🖋️',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Memory Palace',
    description: 'Memorize 5 new items',
    anchor: 'While walking',
    category: 'Intellect',
    emoji: '🏰',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Critical Analysis',
    description: 'Question one assumption',
    anchor: 'Journaling',
    category: 'Intellect',
    emoji: '🔍',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Teach Someone',
    description: 'Explain a new concept',
    anchor: 'Social time',
    category: 'Intellect',
    emoji: '👨‍🏫',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Curiosity Log',
    description: 'Write 3 questions',
    anchor: 'End of day',
    category: 'Intellect',
    emoji: '❓',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Mental Math',
    description: 'Calculate 5 problems',
    anchor: 'Morning shower',
    category: 'Intellect',
    emoji: '🔢',
    attribute: HabitAttribute.intellect,
  ),
  HabitTemplate(
    title: 'Skill Research',
    description: 'Deep dive for 15 mins',
    anchor: 'After lunch',
    category: 'Intellect',
    emoji: '🛠️',
    attribute: HabitAttribute.intellect,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'Philosophy Read',
    description: 'One page of Stoicism',
    anchor: 'Before bed',
    category: 'Intellect',
    emoji: '🏛️',
    attribute: HabitAttribute.intellect,
  ),

  // ═══ FOCUS (Productivity) ═══
  HabitTemplate(
    title: 'Pomodoro Power',
    description: 'One 25-minute sprint',
    anchor: 'Start of workday',
    category: 'Focus',
    emoji: '🍅',
    attribute: HabitAttribute.focus,
    timerDurationMinutes: 25,
  ),
  HabitTemplate(
    title: 'Task Batching',
    description: 'Group all small tasks',
    anchor: 'After lunch',
    category: 'Focus',
    emoji: '🖇️',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Inbox Zero',
    description: 'Clean communication logs',
    anchor: '4:00 PM',
    category: 'Focus',
    emoji: '📧',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Phone-free Work',
    description: 'Device in other room',
    anchor: 'Deep work block',
    category: 'Focus',
    emoji: '📵',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Top 3 Priorities',
    description: 'Define daily wins',
    anchor: 'After waking up',
    category: 'Focus',
    emoji: '🥇',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Workspace Reset',
    description: 'Clear all surfaces',
    anchor: 'End of workday',
    category: 'Focus',
    emoji: '🧹',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Time Tracking',
    description: 'Log one full day',
    anchor: 'Ongoing',
    category: 'Focus',
    emoji: '⏱️',
    attribute: HabitAttribute.focus,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Meeting Prep',
    description: 'Review agenda 5 mins',
    anchor: 'Before calls',
    category: 'Focus',
    emoji: '📑',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Notification Audit',
    description: 'Disable 5 alerts',
    anchor: 'Weekly',
    category: 'Focus',
    emoji: '🔕',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Focus Music',
    description: 'Enable LoFi/Binaural',
    anchor: 'Entering flow',
    category: 'Focus',
    emoji: '🎧',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Say "No" to 1',
    description: 'Reject an interruption',
    anchor: 'Daily challenge',
    category: 'Focus',
    emoji: '⛔',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Weekly Planning',
    description: 'Map next 7 days',
    anchor: 'Sunday night',
    category: 'Focus',
    emoji: '📅',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Energy Audit',
    description: 'Rate energy level',
    anchor: 'After lunch',
    category: 'Focus',
    emoji: '🔋',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'Goal Alignment',
    description: 'Sync tasks with vision',
    anchor: 'Monday morning',
    category: 'Focus',
    emoji: '🏹',
    attribute: HabitAttribute.focus,
  ),
  HabitTemplate(
    title: 'One-Task Mode',
    description: 'No browser tabs except 1',
    anchor: 'Deep work',
    category: 'Focus',
    emoji: '☝️',
    attribute: HabitAttribute.focus,
  ),

  // ═══ CREATIVITY (Expression) ═══
  HabitTemplate(
    title: 'Morning Pages',
    description: '3 pages of free writing',
    anchor: 'After waking up',
    category: 'Creativity',
    emoji: '✍️',
    attribute: HabitAttribute.creativity,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Sketching Break',
    description: '10 minute drawing',
    anchor: 'Lunch break',
    category: 'Creativity',
    emoji: '🎨',
    attribute: HabitAttribute.creativity,
    timerDurationMinutes: 10,
  ),
  HabitTemplate(
    title: 'Musical Improv',
    description: '5 mins on instrument',
    anchor: 'After work',
    category: 'Creativity',
    emoji: '🎸',
    attribute: HabitAttribute.creativity,
    timerDurationMinutes: 5,
  ),
  HabitTemplate(
    title: 'Mood Boarding',
    description: 'Collect 5 images',
    anchor: 'During research',
    category: 'Creativity',
    emoji: '🖼️',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Brainstorming 10',
    description: '10 ideas for 1 problem',
    anchor: 'Morning coffee',
    category: 'Creativity',
    emoji: '💡',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Creative Writing',
    description: '200 words of fiction',
    anchor: 'After dinner',
    category: 'Creativity',
    emoji: '✒️',
    attribute: HabitAttribute.creativity,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'Photography Walk',
    description: 'Take 3 unique photos',
    anchor: 'Weekend morning',
    category: 'Creativity',
    emoji: '📸',
    attribute: HabitAttribute.creativity,
    timerDurationMinutes: 15,
  ),
  HabitTemplate(
    title: 'DIY Project',
    description: '15 mins manual work',
    anchor: 'After work',
    category: 'Creativity',
    emoji: '🛠️',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Design Iteration',
    description: 'Improve one existing work',
    anchor: 'Studio time',
    category: 'Creativity',
    emoji: '🔄',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Poetry Reading',
    description: 'One poem per day',
    anchor: 'Before bed',
    category: 'Creativity',
    emoji: '📜',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Inspiration Log',
    description: 'Note one creative spark',
    anchor: 'Ongoing',
    category: 'Creativity',
    emoji: '✨',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Color Experiment',
    description: 'Pick a palette for today',
    anchor: 'Morning ritual',
    category: 'Creativity',
    emoji: '🌈',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Storyboard Session',
    description: 'Sketch 3 scenes',
    anchor: 'Planning phase',
    category: 'Creativity',
    emoji: '🎞️',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Font/Typography',
    description: 'Study one typeface',
    anchor: 'Evening browse',
    category: 'Creativity',
    emoji: '🅰️',
    attribute: HabitAttribute.creativity,
  ),
  HabitTemplate(
    title: 'Creative Rest',
    description: 'Total visual detox',
    anchor: 'Sundays',
    category: 'Creativity',
    emoji: '🌑',
    attribute: HabitAttribute.creativity,
  ),
];

class HabitTemplateSheet extends StatefulWidget {
  final UserArchetype archetype;
  final OnTemplateSelected onTemplateSelected;

  const HabitTemplateSheet({
    super.key,
    required this.archetype,
    required this.onTemplateSelected,
  });

  @override
  State<HabitTemplateSheet> createState() => _HabitTemplateSheetState();
}

class _HabitTemplateSheetState extends State<HabitTemplateSheet> {
  String _selectedCategory = 'All';

  /// Get the color for a category based on its associated attribute
  Color _getCategoryColor(String category) {
    switch (category) {
      case 'Strength':
        return attributeColor(HabitAttribute.strength);
      case 'Vitality':
        return attributeColor(HabitAttribute.vitality);
      case 'Spirit':
        return attributeColor(HabitAttribute.spirit);
      case 'Intellect':
        return attributeColor(HabitAttribute.intellect);
      case 'Focus':
        return attributeColor(HabitAttribute.focus);
      case 'Creativity':
        return attributeColor(HabitAttribute.creativity);
      default:
        // Use archetype theme color for 'All'
        return ArchetypeTheme.forArchetype(widget.archetype).primaryColor;
    }
  }

  List<HabitTemplate> get _allTemplates => habitTemplateCatalog;

  List<String> get _categories => [
    'All',
    'Strength',
    'Vitality',
    'Spirit',
    'Intellect',
    'Focus',
    'Creativity',
  ];

  List<HabitTemplate> get _filteredTemplates {
    if (_selectedCategory == 'All') {
      return _allTemplates;
    }
    return _allTemplates.where((t) => t.category == _selectedCategory).toList();
  }

  @override
  Widget build(BuildContext context) {
    return DraggableScrollableSheet(
      initialChildSize: 0.75,
      minChildSize: 0.5,
      maxChildSize: 0.95,
      builder: (context, scrollController) {
        return Container(
          decoration: BoxDecoration(
            gradient: const LinearGradient(
              begin: Alignment.topCenter,
              end: Alignment.bottomCenter,
              colors: [Color(0xFF1A1A2E), Color(0xFF0F0F1A)],
            ),
            borderRadius: const BorderRadius.vertical(top: Radius.circular(24)),
            border: Border.all(
              color: Colors.white.withValues(alpha: 0.1),
              width: 1,
            ),
          ),
          child: Column(
            children: [
              // Handle bar
              Container(
                margin: const EdgeInsets.symmetric(vertical: 12),
                width: 40,
                height: 4,
                decoration: BoxDecoration(
                  color: AppTheme.textSecondaryDark.withValues(alpha: 0.3),
                  borderRadius: BorderRadius.circular(2),
                ),
              ),

              // Header
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 20),
                child: Row(
                  children: [
                    Container(
                      padding: const EdgeInsets.all(10),
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          colors: [
                            _getCategoryColor(
                              _selectedCategory,
                            ).withValues(alpha: 0.2),
                            _getCategoryColor(
                              _selectedCategory,
                            ).withValues(alpha: 0.1),
                          ],
                        ),
                        borderRadius: BorderRadius.circular(12),
                        boxShadow: [
                          BoxShadow(
                            color: _getCategoryColor(
                              _selectedCategory,
                            ).withValues(alpha: 0.2),
                            blurRadius: 12,
                            spreadRadius: -2,
                          ),
                        ],
                      ),
                      child: Icon(
                        Icons.auto_stories,
                        color: _getCategoryColor(_selectedCategory),
                        size: 22,
                      ),
                    ),
                    const Gap(14),
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          'Habit Templates',
                          style: Theme.of(context).textTheme.titleLarge
                              ?.copyWith(
                                color: AppTheme.textMainDark,
                                fontWeight: FontWeight.bold,
                              ),
                        ),
                        const Gap(2),
                        Text(
                          'Tap to quick-fill your habit',
                          style: Theme.of(context).textTheme.bodySmall
                              ?.copyWith(color: AppTheme.textSecondaryDark),
                        ),
                      ],
                    ),
                  ],
                ),
              ),
              const Gap(20),

              // Category chips
              SizedBox(
                height: 40,
                child: ListView.separated(
                  scrollDirection: Axis.horizontal,
                  padding: const EdgeInsets.symmetric(horizontal: 20),
                  itemCount: _categories.length,
                  separatorBuilder: (_, _) => const Gap(10),
                  itemBuilder: (context, index) {
                    final category = _categories[index];
                    final isSelected = category == _selectedCategory;
                    return GestureDetector(
                      onTap: () {
                        HapticFeedback.selectionClick();
                        setState(() => _selectedCategory = category);
                      },
                      child: AnimatedContainer(
                        duration: const Duration(milliseconds: 200),
                        padding: const EdgeInsets.symmetric(
                          horizontal: 18,
                          vertical: 10,
                        ),
                        decoration: BoxDecoration(
                          gradient: isSelected
                              ? LinearGradient(
                                  colors: [
                                    _getCategoryColor(category),
                                    _getCategoryColor(
                                      category,
                                    ).withValues(alpha: 0.8),
                                  ],
                                )
                              : null,
                          color: isSelected
                              ? null
                              : Colors.white.withValues(alpha: 0.05),
                          borderRadius: BorderRadius.circular(20),
                          border: Border.all(
                            color: isSelected
                                ? _getCategoryColor(category)
                                : Colors.white.withValues(alpha: 0.1),
                          ),
                          boxShadow: isSelected
                              ? [
                                  BoxShadow(
                                    color: _getCategoryColor(
                                      category,
                                    ).withValues(alpha: 0.3),
                                    blurRadius: 12,
                                    spreadRadius: -2,
                                  ),
                                ]
                              : null,
                        ),
                        child: Text(
                          category,
                          style: TextStyle(
                            color: isSelected
                                ? Colors.white
                                : AppTheme.textSecondaryDark,
                            fontWeight: FontWeight.w600,
                            fontSize: 13,
                          ),
                        ),
                      ),
                    );
                  },
                ),
              ),
              const Gap(20),

              // Template grid
              Expanded(
                child: ListView.separated(
                  controller: scrollController,
                  padding: const EdgeInsets.fromLTRB(20, 0, 20, 40),
                  itemCount: _filteredTemplates.length,
                  separatorBuilder: (_, _) => const Gap(12),
                  itemBuilder: (context, index) {
                    final template = _filteredTemplates[index];
                    return _TemplateListTile(
                      template: template,
                      accentColor: template.color,
                      onTap: () {
                        HapticFeedback.mediumImpact();
                        widget.onTemplateSelected(template);
                      },
                    );
                  },
                ),
              ),
            ],
          ),
        );
      },
    );
  }
}

/// List tile for bottom sheet templates with glassmorphism
class _TemplateListTile extends StatelessWidget {
  final HabitTemplate template;
  final Color accentColor;
  final VoidCallback onTap;

  const _TemplateListTile({
    required this.template,
    required this.accentColor,
    required this.onTap,
  });

  @override
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: Colors.white.withValues(alpha: 0.05),
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: Colors.white.withValues(alpha: 0.08)),
        ),
        child: Row(
          children: [
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [
                    accentColor.withValues(alpha: 0.2),
                    accentColor.withValues(alpha: 0.1),
                  ],
                ),
                borderRadius: BorderRadius.circular(14),
              ),
              child: Text(template.emoji, style: const TextStyle(fontSize: 22)),
            ),
            const Gap(16),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(
                    template.title,
                    style: Theme.of(context).textTheme.titleSmall?.copyWith(
                      color: AppTheme.textMainDark,
                      fontWeight: FontWeight.w600,
                    ),
                  ),
                  const Gap(4),
                  Text(
                    template.description,
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: AppTheme.textSecondaryDark,
                    ),
                  ),
                  const Gap(8),
                  Wrap(
                    spacing: 8,
                    runSpacing: 4,
                    children: [
                      _InfoChip(
                        icon: Icons.link,
                        label: template.anchor,
                        color: accentColor,
                      ),
                      _InfoChip(
                        icon: Icons.schedule,
                        label: template.timeOfDay,
                        color: accentColor,
                      ),
                    ],
                  ),
                ],
              ),
            ),
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: accentColor.withValues(alpha: 0.1),
                borderRadius: BorderRadius.circular(10),
              ),
              child: Icon(Icons.add, color: accentColor, size: 20),
            ),
          ],
        ),
      ),
    );
  }
}

/// Small info chip for template details
class _InfoChip extends StatelessWidget {
  final IconData icon;
  final String label;
  final Color color;

  const _InfoChip({
    required this.icon,
    required this.label,
    required this.color,
  });

  @override
  Widget build(BuildContext context) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
        color: color.withValues(alpha: 0.08),
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: color.withValues(alpha: 0.15)),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 11, color: color.withValues(alpha: 0.8)),
          const Gap(4),
          Text(
            label,
            style: TextStyle(
              fontSize: 10,
              color: color.withValues(alpha: 0.9),
              fontWeight: FontWeight.w500,
            ),
          ),
        ],
      ),
    );
  }
}
