    ...
    StreamProvider.autoDispose<List<FriendRankEntry>>((ref) {
      ...
      return firestore
          ...
          .asyncMap((friendsSnapshot) async {
            ...
            for (var i = 0; i < friendIds.length; i += batchSize) {
              ...
              try {
                ...
                for (final doc in statsSnapshot.docs) {
                  ...
                  try {
                    friendDoc = friendsSnapshot.docs.firstWhere(
                      (d) => d.id == doc.id,
                    );
                  } catch (_) {
                    friendDoc = null;
                  ...
                  if (friendDoc == null) {
                    AppLogger.w(
                      'Friend ${doc.id} found in user_stats but not in friends collection - skipping',
                    );
                    continue;
                  }
                  ...
                  final avatarStats =
                      data['avatarStats'] as Map<String, dynamic>?;
                  int totalXp = 0;

                  if (avatarStats != null) {
                    // Add type validation for each attribute
                    totalXp += _safeInt(
                      avatarStats['strengthXp'],
                      doc.id,
                      'strengthXp',
                    );
                    totalXp += _safeInt(
                      avatarStats['intellectXp'],
                      doc.id,
                      'intellectXp',
                    );
                    totalXp += _safeInt(
                      avatarStats['vitalityXp'],
                      doc.id,
                      'vitalityXp',
                    );
                    totalXp += _safeInt(
                      avatarStats['creativityXp'],
                      doc.id,
                      'creativityXp',
                    );
                    totalXp += _safeInt(
                ...
                AppLogger.e(
                  'Error loading friends leaderboard batch',
                  e,
                  stack,
                );
            ...
            try {
              final myStatsRef = await firestore
                  .collection('user_stats')
                  .doc(user.id)
                  .get();
              if (myStatsRef.exists) {
                ...
                final myAvatarStats =
                    myData['avatarStats'] as Map<String, dynamic>?;
                int myXp = 0;

                if (myAvatarStats != null) {
                  myXp += _safeInt(
                    myAvatarStats['strengthXp'],
                    user.id,
                    'strengthXp',
                  );
                  myXp += _safeInt(
                    myAvatarStats['intellectXp'],
                    user.id,
                    'intellectXp',
                  );
                  myXp += _safeInt(
                    myAvatarStats['vitalityXp'],
                    user.id,
                    'vitalityXp',
                  );
                  myXp += _safeInt(
                    myAvatarStats['creativityXp'],
                    user.id,
                    'creativityXp',
                  );
                  myXp += _safeInt(
                    myAvatarStats['focusXp'],
                    user.id,
                    'focusXp',
                  );
                  myXp += _safeInt(
                    myAvatarStats['spiritXp'],
                    user.id,
                    'spiritXp',
                  );
                }
              ...
              AppLogger.e(
                'Error loading current user stats for leaderboard',
                e,
                stack,
              );
            }
...
int _safeInt(dynamic value, String userId, String fieldName) {
  ...
  AppLogger.w(
    'Invalid type for $fieldName for user $userId: ${value.runtimeType}',
  );
  return 0;
}
